---
name: creative-diagram
description: >
  สร้างไดอะแกรมแบบมืออาชีพ 39 ชนิด (architecture, flowchart, sequence, state machine,
  ER/data model, timeline, swimlane, quadrant, radar, org chart, Gantt, Sankey, Wardley map,
  db schema ฯลฯ) เป็นไฟล์ HTML/SVG แบบ self-contained ตาม editorial design system
  ที่ปรับแบรนด์ได้ (ดึง token จากเว็บไซต์, ตั้ง profile ต่อโปรเจกต์) รองรับ redraw จากไฟล์ .drawio หรือ
  Mermaid .mmd, ใส่ motion/animation, สไตล์ sketchy หรือ terminal ได้ด้วย เรียกใช้ผ่าน
  `/guide:creative-diagram` เท่านั้น — ไม่ auto-trigger จากบทสนทนา
...
4. **วาดตาม design system ใน §5–§8** — สี/ตัวอักษรอ้างอิงจาก `references/style-guide.md` เสมอ
   (ไม่ hardcode ทับ) เลือก variant จาก `assets/` (minimal light/dark, full editorial, terminal,
   sketchy) ตามบริบทที่ใช้งาน
5. **รัน pre-output checklist ใน §9 ก่อนส่งมอบทุกครั้ง** — *หลักการ: กฎเส้นเชื่อม 6 ข้อ (§6) และ
   accessible-SVG contract เป็นจุดที่พังบ่อยที่สุดถ้าไม่เช็คซ้ำ*
6. **ถ้าผู้ใช้ให้ไฟล์ `.drawio*` หรือ Mermaid `.mmd`/fenced block มา** ให้เปิด
   `references/import-drawio.md` หรือ `references/import-mermaid.md` ตามชนิดไฟล์ — extract โครงสร้าง
   ด้วย script ก่อนเสมอ อย่าอ่านไฟล์ต้นทางตรงๆ ด้วย Read (โดยเฉพาะ `.drawio` ที่มักเป็น
   deflate+base64)
7. **ถ้าผู้ใช้ขอ export เป็น PNG/SVG** ให้เปิด `references/export.md` — เป็นขั้นตอนที่ต้องทำเมื่อขอ
   เท่านั้น ห้าม export เองโดยไม่มีใครขอ
8. **ถ้าผู้ใช้ขอ diagnostics/health check** ให้เปิด `references/doctor.md`

**เกี่ยวกับการรัน script:** ทุกคำสั่ง `python3 scripts/...` ที่ปรากฏใน SKILL.md นี้และใน
`references/*.md` ให้ลองรันตามลำดับ `python3` → `py` → `python` จนกว่าจะได้ผลลัพธ์จริง (ไม่ใช่ error
"command not found" หรือ Microsoft Store alias เปล่าบน Windows) — ใช้ path แบบ absolute ต่อจาก
base directory ของ skill นี้เสมอ ไม่พึ่ง relative path

# คำขอ:

- ห้ามข้ามงบความซับซ้อน (§7) หรือกฎเส้นเชื่อม 6 ข้อ (§6) แม้ผู้ใช้จะรีบ — ถ้าเนื้อหาเกินงบ ให้แยกเป็น
  ภาพรวม + รายละเอียดแทนการยัดทุกอย่างลงภาพเดียว
- ห้ามใช้สีตรงๆ (hex code) แทนที่จะอ้าง semantic role จาก `style-guide.md` — เพราะจะทำให้ปรับแบรนด์
  ทีหลังไม่ได้
- ห้ามสร้างไฟล์ export (PNG/SVG) โดยผู้ใช้ไม่ได้ขอ
- ก่อนวาดไดอะแกรมแรกในโปรเจกต์ที่ยังไม่เคยตั้ง style guide ห้ามข้าม gate ใน §0 ไปเงียบๆ
- ไฟล์ reference และ asset ทั้งหมดในสคีลนี้เป็นภาษาอังกฤษโดยตั้งใจ (เป็น spec ทางเทคนิคที่มีค่าตัวเลข/
  โค้ด CSS/SVG เจาะจงมาก การแปลจะเสี่ยงคลาดเคลื่อน) — ส่วนที่เป็นภาษาไทยคือหน้า SKILL.md นี้สำหรับ
  ให้คนอ่านเข้าใจภาพรวม

# ไฟล์แนบ:

โหลด reference เฉพาะตอนที่เกี่ยวข้อง (ไม่ใช่โหลดทุกไฟล์ทุกครั้ง) — ดูตาราง 39 ชนิดไดอะแกรมแบบเต็มใน
§3 ของ engine spec ด้านล่างสำหรับ mapping ชนิด → ไฟล์ `references/type-*.md` ไฟล์สนับสนุนอื่นๆ:

| ไฟล์ | เปิดเมื่อ |
|---|---|
| `references/style-guide.md` | ทุกครั้งที่วาด — เป็นแหล่งความจริงของสี/ตัวอักษร |
| `references/semantic-patterns.md` | พฤติกรรม/สถานะ/ความเสี่ยงเป็นตัวขับความหมายของภาพ |
| `references/onboarding.md` | ต้องดึง brand token จากเว็บไซต์/ไฟล์ในเครื่อง |
| `references/profiles.md` | ต้องจัดการ client profile หลายชุด (save/load/list/delete) |
| `references/animation.md` | ผู้ใช้ขอ motion/animation อย่างชัดเจน |
| `references/primitive-annotation.md` | ต้องใส่ editorial callout |
| `references/primitive-sketchy.md` | ต้องการสไตล์ hand-drawn |
| `references/primitive-icons.md` | ต้องใช้ icon set (ดูตัวอย่างที่ `assets/icons.html`) |
| `references/primitive-terminal.md` | ต้องการสไตล์ terminal/CLI window |
| `references/import-drawio.md` | ต้นทางเป็นไฟล์ `.drawio*` |
| `references/import-mermaid.md` | ต้นทางเป็น Mermaid `.mmd`/`.mermaid`/fenced block |
| `references/output-spec.md` | ต้องตั้งค่า format/size/detail/audience สำหรับงาน import |
| `references/export.md` | ผู้ใช้ขอ export เป็น PNG/SVG |
| `references/doctor.md` | ผู้ใช้ขอ diagnostics/health check |
| `scripts/drawio_extract.py`, `scripts/mermaid_extract.py` | extract โครงสร้างจากไฟล์ต้นทางก่อน redraw |
| `scripts/self_check.py` | ตรวจไฟล์ที่สร้างเสร็จแล้วก่อนส่งมอบ (accessible-SVG, motion contract) |
| `assets/template*.html`, `assets/example-*.html` | จุดเริ่มต้น/ตัวอย่างอ้างอิงต่อ variant และต่อชนิด |

---

# Engine spec (เนื้อหาต้นฉบับภาษาอังกฤษ — โหลด/อ่านส่วนนี้เสมอเมื่อ skill ถูกเรียก)
...
- [ ] From this skill's directory, did `python3 scripts/self_check.py <file>` pass (try `py`/`python` if `python3` doesn't resolve)? (Accessible-SVG contract, single-file safety, motion basics.)
