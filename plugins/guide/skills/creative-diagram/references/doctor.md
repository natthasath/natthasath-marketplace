...
Load this file when the user asks to run diagnostics, health checks, or first-run troubleshooting for this skill's import/export tooling.
...
1. Python runtime
- Resolve `python3` first, then `py`, then `python` (Windows often ships `python3`/`python` as empty Microsoft Store aliases — try each until one actually runs).
...
- Re-run: ask for diagnostics again with --strict
```
