...
**ทุก skill ในนี้ต้องมีหัวข้อหลัก 4 หัวข้อนี้เสมอ เรียงตามลำดับนี้ ห้ามข้ามหรือเปลี่ยนชื่อ** — นี่คือสิ่งที่
ทำให้ skill ที่ copycat สร้างแยกไม่ออกจาก skill อื่นในนี้ทันทีที่เปิดอ่าน:

| หัวข้อ | ใช้ทำอะไร |
|---|---|
| `# บทบาท:` | อธิบายว่า Claude รับบทบาทอะไร + ทำไมงานนี้ถึงมีคุณค่า (ย่อหน้าสั้นๆ ไม่ใช่แค่ "คุณคือ X") |
| `# รูปแบบ:` | ทุกอย่างที่เกี่ยวกับ "ทำยังไง"/"ออกมาหน้าตาแบบไหน" — ลำดับขั้นตอน, กฎการเลือก, template ของ output ใส่รวมในหัวข้อนี้เป็น `##` sub-header ย่อยได้ตามต้องการ (เช่น `## วิธีเลือก Tags`, `## 1. สร้าง design tree`, `## Output`) แต่ละข้อควรมี *หลักการ: ...* กำกับสั้นๆ ว่าทำไมข้อนี้ถึงสำคัญ แทนการเขียน MUST/ห้าม แบบไม่มีเหตุผล ถ้า output มีรูปแบบตายตัว ใส่ template ใน code fence พร้อมตัวอย่าง input → output จริงอย่างน้อย 1 ชุด |
| `# คำขอ:` | กฎที่ต้องทำตามเสมอ เขียนเป็น bullet คำสั่ง (imperative) |
| `# ไฟล์แนบ:` | บอกว่าผู้ใช้ต้องส่งอะไรมาให้ skill นี้ทำงาน (ข้อความ, ไฟล์, ลิงก์) และถ้า skill มี reference/asset ของตัวเอง ให้บอกว่ามีไฟล์อะไรบ้างและเมื่อไหร่ต้องเปิดอ่านด้วย |

เดิม repo นี้เคยใช้ `# ขั้นตอน:` แทน `# รูปแบบ:` สำหรับ skill ที่เน้นลำดับขั้นตอนมากกว่า spec ของ output —
ตอนนี้รวมเป็น `# รูปแบบ:` หัวข้อเดียวเสมอ (ขั้นตอนก็ถือเป็น "รูปแบบการทำงาน" แบบหนึ่ง) เพื่อไม่ให้ผู้อ่าน
ต้องจำสองชื่อหัวข้อที่ทำหน้าที่เดียวกัน — ถ้าเจอ skill เก่าที่ยังใช้ `# ขั้นตอน:` อยู่ ให้ถือเป็นของที่ยัง
ไม่ได้อัปเดต ไม่ใช่ pattern ที่ยังใช้ได้
