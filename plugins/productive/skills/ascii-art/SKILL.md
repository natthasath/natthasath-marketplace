...
## 3. ถามผู้ใช้ว่าอยากได้แบบไหน

ใช้ `AskUserQuestion` ถามพร้อมกันทีเดียว อย่าถามทีละข้อ — ตัวเลือกพวกนี้ไม่ขึ้นต่อกัน
การถามทีละคำถามทำให้ผู้ใช้ต้องรอหลายรอบโดยไม่ได้อะไรเพิ่ม

**ถ้า input เป็นข้อความ** ให้ถาม 2 เรื่อง:

| คำถาม | ตัวเลือกที่ควรเสนอ |
|---|---|
| โหมด | `figlet` (ตัวใหญ่ล้วน) / `toilet` (ตัวใหญ่ + สี + กรอบ) / `cowsay` (ตัวการ์ตูนพูด) / `box` (กรอบล้อมข้อความธรรมดา) |
| สไตล์ฟอนต์ + สี | เสนอเป็นคู่ที่เข้ากันจริง เช่น "ansi_shadow + rainbow (โมเดิร์น เด่น)", "slant + cyan (คลาสสิก สะอาด)", "bloody + fire (ดุ ดิบ)", "pagga + matrix (แน่น เท่)" |

**ถ้า input เป็นรูป** ให้ถาม 2 เรื่อง:

| คำถาม | ตัวเลือกที่ควรเสนอ |
|---|---|
| โหมด | `jp2a` (ASCII ล้วน คัดลอกไปแปะที่ไหนก็ได้) / `chafa` (บล็อกสีจริง ใกล้เคียงรูปต้นฉบับที่สุด แต่ใช้ได้แค่ในเทอร์มินัล) / `braille` (ละเอียดสูง ขาวดำ) |
| ความกว้าง | 40 (ย่อ) / 80 (มาตรฐาน) / 120 (ละเอียด) |

**ข้ามคำถามข้อไหนที่ผู้ใช้ระบุมาแล้ว** — ถ้าเขาพิมพ์ว่า "ทำ Hello เป็น figlet ฟอนต์ slant"
ก็รันได้เลย ไม่ต้องถามซ้ำ การถามสิ่งที่เขาเพิ่งบอกไปทำให้รู้สึกว่าไม่ได้ฟัง
...
| `--out PATH` | ทุกโหมด | บันทึกลงไฟล์ด้วย (`.txt` ถ้าไม่มีสี, `.ans` ถ้ามีสี) |
...
- **ถามก่อนรันเสมอ ยกเว้นผู้ใช้ระบุมาครบแล้ว** — และถามพร้อมกันทีเดียวด้วย `AskUserQuestion`
  ไม่ใช่ถามทีละข้อ
- **เสนอตัวเลือกเป็นคู่ที่เข้ากัน ไม่ใช่ dump รายชื่อ 571 ฟอนต์** — คนที่ไม่เคยใช้ figlet
  ไม่รู้ว่า `larry3d` หน้าตายังไง การเสนอ "ansi_shadow + rainbow (โมเดิร์น เด่น)" ช่วยให้
  เลือกได้จากความรู้สึกที่อยากได้ ซึ่งเป็นสิ่งที่เขารู้จริง
